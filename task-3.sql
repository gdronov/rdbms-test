...
SELECT
    ...
    bond_id,
    avg_bid,
    AVG(avg_bid) OVER w AS roll_avg_bid,
    avg_ask,
    AVG(avg_ask) OVER w AS roll_avg_ask
FROM (
    SELECT
        period.trading_date,
        bonds.id AS bond_id,
    ...
    FROM period
    JOIN bonds
    LEFT JOIN exch_quotes_archive AS eqa
         ON (eqa.trading_date = period.trading_date AND eqa.bond_id = bonds.id)
    GROUP BY period.trading_date, bonds.id
) AS av
WINDOW w AS (
  PARTITION BY bond_id
  ORDER BY bond_id, trading_date
  ROWS BETWEEN 6 PRECEDING AND CURRENT ROW -- Окно 7 дней
)
ORDER BY trading_date, bond_id
